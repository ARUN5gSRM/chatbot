...
import psycopg2
import requests
from cb_app.forms import RegisteredTicketForm
from cb_app.models import RegisteredTicket, Ticket
...
# ----------------- Register Ticket -----------------
@login_required
def register_ticket_view(request):
    if request.method == "POST":
        form = RegisteredTicketForm(request.POST)
        if form.is_valid():
            ...
            messages.success(
                request,
                f"✅ Ticket created successfully! Your Ticket ID is {ticket.ticket_id}"
            )
            # Redirect to success page to prevent form resubmission
            return redirect("cb_app:register_ticket_success", ticket_id=ticket.ticket_id)
        else:
            messages.error(request, "❌ Please correct the errors below.")
    else:
        form = RegisteredTicketForm()

    return render(request, "register_ticket.html", {"form": form})
    ...
    if request.method == "POST" and request.FILES.get("file"):
        ...
        try:
            ...
            def parse_resolution_notes(notes):
                ...
                if sol_match: solution = sol_match.group(1).strip()
                elif notes: solution = str(notes).strip()
