...
import re
from .ollama_helper import ensure_ollama_running, generate_rag_response
...
def load_tickets():
    engine = get_engine()
    df = pd.read_sql(
        "SELECT short_description, resolution_code, description, keywords, solution FROM tickets_final",
        engine
    )
...
def chatbot_search(query, top_k=3):
    ...
    # --- LLM Structured RAG (Human-readable, like other results) ---
    try:
        ensure_ollama_running()
        top_tickets = results["tfidf"]
        ...
        rag_output = generate_rag_response(rag_prompt)
